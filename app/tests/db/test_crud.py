...
def response(create_single_forecast):
    ...
    forecasts = session.query(models.SingleForecast).filter(models.SingleForecast.weather_station == station_code).filter(
        models.SingleForecast.issue_date == issue_date_datetime).all()
